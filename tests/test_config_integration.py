...
from hci_extractor.core.config import ExtractorConfig, AnalysisConfig, RetryConfig
from hci_extractor.core.analysis import LLMSectionProcessor, detect_sections
from hci_extractor.core.models import PdfContent, PdfPage, DetectedSection
...
The results section has a reasonable amount of content that should be detected with lower thresholds but might be filtered with very high ones."""
    ...
    # With lenient config, all sections should be detected
    assert len(sections_30) >= 3  # At least abstract, intro, and one more
    
    # With strict config, short sections should be filtered out
    assert len(sections_150) < len(sections_30)
