    ...
    def test_extract_command_requires_api_key(self):
        ...
        with tempfile.NamedTemporaryFile(suffix=".pdf") as tmp_pdf:
            ...
            with patch.dict("os.environ", {}, clear=True):
                ...
                assert result.exit_code != 0
                # Check for user-friendly error message (our error translator converts technical errors)
                assert (
                    "error occurred" in result.output.lower()
                    or "resource issue" in result.output.lower()
                )
