...
class TestSimpleExtraction:
    ...
    async def test_extract_paper_simple_basic(self):
        ...
        try:
            ...
            with patch(
                "hci_extractor.core.extraction.PdfExtractor.extract_content",
    ...
    async def test_extract_multiple_papers(self):
        ...
        try:
            ...
            assert any(
                "paper 0" in title.lower() or "paper_0" in title.lower()
                for title in titles
            )
            assert any(
                "paper 1" in title.lower() or "paper_1" in title.lower()
                for title in titles
            )
...
class TestExtractionErrorHandling:
    ...
    async def test_extract_paper_missing_file(self):
        ...
        with pytest.raises((FileNotFoundError, Exception)):
            await extract_paper_simple(pdf_path=missing_path, llm_provider=mock_llm)
