...
class TestGlobalStateViolations:
    ...
    def test_no_global_container_instance(self):
        ...
        global_vars = {k: v for k, v in module_vars.items() 
                      if not k.startswith('_') and not inspect.isfunction(v) and not inspect.isclass(v)}
        
        # Filter out expected constants and imports
        expected_globals = {
            'DIContainer', 'ExtractorConfig', 'EventBus', 'LLMProvider', 'GeminiProvider', 
            'ProviderConfigurationPort', 'ExtractorConfigurationAdapter',
            # Standard imports and typing
            'Type', 'Optional', 'logger', 'asyncio', 'Dict', 'T', 'Union', 'logging', 'Callable'
        }
        actual_globals = set(global_vars.keys()) - expected_globals
        
        assert not actual_globals, f"Found unexpected global variables: {actual_globals}"

    def test_no_get_container_function(self):
        """Test that get_container() function has been removed."""
        from hci_extractor.core import di_container
        
        assert not hasattr(di_container, 'get_container'), "get_container() function should be removed"
    ...
    def test_cli_configuration_service_no_global_state(self):
        ...
        # Services should be the same instance (singleton pattern for CLI)
        # but should not maintain mutable state
        assert service1 is service2, "CLI service should be singleton"
...
class TestImmutableDesignViolations:
    ...
    def test_dataclass_frozen_compliance(self):
        ...
        for dataclass_type in dataclasses_to_check:
            assert is_dataclass(dataclass_type), f"{dataclass_type.__name__} should be a dataclass"
            ...
            if hasattr(dataclass_type, '__dataclass_params__'):
                ...
                assert params.frozen, f"{dataclass_type.__name__} should be frozen (use @dataclass(frozen=True))"
...
class TestHexagonalArchitectureCompliance:
    ...
    def test_configuration_service_separation(self):
        ...
        assert ConfigurationService.__module__.startswith('hci_extractor.infrastructure'), \
            "ConfigurationService should be in infrastructure layer"
        
        # Test that it has proper methods for environment access
        assert hasattr(ConfigurationService, 'load_from_environment'), \
            "ConfigurationService should have load_from_environment method"
...
class TestProviderArchitectureCompliance:
    ...
    def test_provider_configuration_abstraction(self):
        ...
        assert is_dataclass(LLMProviderConfig), "LLMProviderConfig should be a dataclass"
        
        # Test that it has expected fields
        config_fields = {f.name for f in fields(LLMProviderConfig)}
        expected_fields = {'api_key', 'temperature', 'max_output_tokens', 'max_attempts'}
    ...
    def test_provider_immutable_state(self):
        ...
        for attr_name, attr_value in provider_vars.items():
            if isinstance(attr_value, (list, dict, set)):
                pytest.fail(f"Provider has mutable attribute {attr_name}: {type(attr_value)}")
...
class TestEventBusArchitectureCompliance:
        ...
        for attr_name in bus1_vars:
            if attr_name in bus2_vars:
                ...
                if isinstance(attr1, (list, dict, set)):
                    assert attr1 is not attr2, f"EventBus instances share mutable state: {attr_name}"
    ...
    def test_domain_events_immutable(self):
        ...
        for event_class in event_classes:
            assert is_dataclass(event_class), f"{event_class.__name__} should be a dataclass"
        ...
        for file_path in python_files:
            with open(file_path, 'r') as f:
                ...
                for line_num, line in enumerate(lines, 1):
                    ...
                    stripped = line.strip()
                    if 'TODO' in line.upper() and not stripped.startswith('#') and 'TODO' in stripped:
                        # Only fail if it's not in a comment context
                        if not any(comment_marker in line for comment_marker in ['#', '"""', "'''"]):
                            pytest.fail(f"TODO comment found in {file_path}:{line_num}: {line.strip()}")
