...
class TestClassificationPrecision:
            ...
            for item in ALL_GROUND_TRUTH:
                if item["section"] == section_type:
                    results.append({
                        ...
                        "confidence": item["confidence_threshold"] + 0.1  # Slightly above threshold
                    })
            return results
        ...
        for section_type, items in sections_data.items():
            ...
            for element in elements:
                ...
                if matching_truth:
                    ...
                    if (element.element_type == matching_truth["expected_element_type"] and
                        element.evidence_type == matching_truth["expected_evidence_type"]):
                        correct_classifications += 1
        
        # Calculate precision
        precision = correct_classifications / total_classifications if total_classifications > 0 else 0
        
        # Should meet 90% precision requirement
        assert precision >= 0.90, f"Classification precision {precision:.2%} below 90% requirement"
    ...
    async def test_evidence_type_classification_precision(self):
        ...
        paper = Paper.create_with_auto_id(title="Evidence Test", authors=("Dr. Evidence",))
        ...
        for item in ALL_GROUND_TRUTH:
            ...
            if expected_evidence in evidence_type_accuracy:
                ...
                for element in elements:
                    if (element.text == item["text"] and 
                        element.evidence_type == expected_evidence):
        ...
        for evidence_type, stats in evidence_type_accuracy.items():
            if stats["total"] > 0:
                ...
                assert precision >= 0.85, f"{evidence_type} evidence classification precision {precision:.2%} too low"
        ...
        for case in edge_cases:
            ...
            for element in elements:
                if case["is_ambiguous"]:
                    assert element.confidence <= 0.80, f"Ambiguous case should have lower confidence: {element.text}"
                else:
                    assert element.confidence >= 0.85, f"Clear case should have higher confidence: {element.text}"
    ...
    def test_classification_statistics(self):
        ...
        ground_truth = [
            ...
            {"element_type": "finding", "evidence_type": "quantitative"},  # Second should be finding
    ...
    async def test_classification_consistency(self):
        ...
        paper = Paper.create_with_auto_id(title="Consistency Test", authors=("Dr. Consistent",))
        section = DetectedSection.create_with_auto_id(
...
class TestClassificationBenchmarking:
    ...
    def test_academic_classification_standards(self):
        ...
        assert classification_standards["target_precision"] > classification_standards["minimum_precision"]
        assert classification_standards["confidence_threshold"] < classification_standards["minimum_precision"]
