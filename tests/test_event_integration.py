...
from hci_extractor.core.analysis import extract_paper_simple, LLMSectionProcessor
...
def event_collector():
    ...
    event_bus.clear()  # Clear any existing handlers
        ...
        instance.extract_content.return_value = PdfContent(
            ...
            pages=(
                PdfPage(
                    ...
                    char_count=len("Title of the Paper\n\nAbstract\n\nThis is a test abstract with findings and results that should be detected by the section detector.\n\n1. Introduction\n\nThis is the introduction section with background information."),
...
@pytest.mark.asyncio
async def test_extraction_failure_events(
    event_collector,
    mock_llm_provider,
    tmp_path
):
...
@pytest.mark.asyncio
async def test_section_processing_events(
    event_collector,
    mock_llm_provider
):
...
@pytest.mark.asyncio
async def test_chunked_section_events(
    event_collector,
    mock_llm_provider
):
    ...
    processor = LLMSectionProcessor(mock_llm_provider, chunk_size=1000, chunk_overlap=100)
    elements = await processor.process_section(section, paper)
    ...
    completed_events = event_collector.get_events_by_type(SectionProcessingCompleted)
    assert len(completed_events) == 1
    assert completed_events[0].elements_extracted > 0  # Should have extracted elements from chunks
