...
class TestLLMMarkupGeneration:
    ...
    @pytest.mark.asyncio
    async def test_generate_markup_success(self, provider_config, mock_event_bus, sample_academic_text, mock_genai):
        """Test successful markup generation."""
        mock_genai_module, mock_model = mock_genai
        
        # Create provider with mock dependencies
        mock_prompt_loader = MagicMock()
        mock_prompt_loader.get_markup_prompt.return_value = "Generate markup for: {text}"
        
        # Mock the API response
        mock_response = MagicMock()
        mock_response.text = """<goal confidence="0.92">develop automated systems</goal> sample text"""
    ...
    @pytest.mark.asyncio
    async def test_generate_markup_with_chunking(self, provider_config, mock_event_bus, mock_genai):
    ...
    @pytest.mark.asyncio
    async def test_generate_markup_api_failure(self, provider_config, mock_event_bus, sample_academic_text, mock_genai):
    ...
    @pytest.mark.asyncio
    async def test_empty_text_handling(self, provider_config, mock_event_bus, mock_genai):
