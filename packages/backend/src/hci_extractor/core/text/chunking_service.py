...
class SentenceBasedChunking(IChunkingStrategy):
    """Sentence-aware chunking that preserves content integrity for markup generation."""
    ...
    def chunk_text(self, text: str, max_chunk_size: int) -> List[str]:
        ...
        for paragraph in paragraphs:
            ...
            if len(paragraph) > max_chunk_size:
                ...
                sentence_chunks = self._process_large_paragraph(paragraph, max_chunk_size)
                result_chunks.extend(sentence_chunks)
                
            elif len(current_chunk) + len(paragraph) + 2 > max_chunk_size and current_chunk:
    ...
    def _process_large_paragraph(self, paragraph: str, max_chunk_size: int) -> List[str]:
        ...
        for sentence in sentences:
            ...
            elif len(current_subchunk) + len(sentence) + 1 > max_chunk_size and current_subchunk:
...
class MarkupChunkingService:
    ...
    def prepare_chunks_for_markup(
        ...
        """
        logger.info(f"Preparing chunks for markup: {len(text)} chars, max_chunk={max_chunk_size}")
        ...
        base_chunks = self.strategy.chunk_text(text, max_chunk_size - overlap_size)
...
def create_markup_chunking_service(mode: ChunkingMode = ChunkingMode.SENTENCE_BASED) -> MarkupChunkingService:
