...
def validate_extracted_elements(
    ...
    logger.info(f"✅ Verbatim check: {len(verified_elements)}/{len(elements)} elements verified")
    ...
    logger.info(f"✅ Quality filter: {len(quality_elements)} elements pass quality check")
...
def quick_validation_stats(
    ...
    return {
        ...
        "sections_represented": len(set(e.section for e in validated_elements)),
        "element_types": {
            element_type: len([e for e in validated_elements if e.element_type == element_type])
