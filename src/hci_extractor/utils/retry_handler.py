...
from hci_extractor.core.config import RetryConfig as ConfigRetryConfig, get_config
from hci_extractor.core.events import (
    get_event_bus, DomainEvent, RetryAttempted, RetryExhausted, RetrySucceeded
)
...
class RetryPolicy:
    ...
    def from_config(cls, config: Optional[ConfigRetryConfig] = None) -> 'RetryPolicy':
        ...
        config = config or get_config().retry
...
class RetryHandler:
        ...
        **kwargs: Any
        ...
        for attempt in range(self._policy.max_attempts):
            try:
                ...
                if self._publish_events and attempt > 0:
                    self._event_bus.publish(RetrySucceeded(
                ...
                if self._publish_events:
                    self._event_bus.publish(RetryAttempted(
                        operation_name=self._operation_name,
        ...
        if self._publish_events:
            self._event_bus.publish(RetryExhausted(
                operation_name=self._operation_name,
    ...
    def _should_not_retry(self, exception: Exception) -> bool:
        ...
        if self._policy.non_retryable_exceptions and isinstance(exception, self._policy.non_retryable_exceptions):
            return True
        
        # Check if it's in the retryable exceptions
        if self._policy.retryable_exceptions and not isinstance(exception, self._policy.retryable_exceptions):
            return True
    ...
    def _calculate_delay(self, attempt: int, exception: Exception) -> float:
        ...
        if self._policy.strategy == RetryStrategy.EXPONENTIAL_BACKOFF:
            delay = self._policy.initial_delay_seconds * (self._policy.backoff_multiplier ** attempt)
