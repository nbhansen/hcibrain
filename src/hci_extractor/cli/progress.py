...
class ExtractionProgress:
    ...
    def with_current_paper(self, paper_name: str, total_sections: int) -> "ExtractionProgress":
...
class ProgressTracker:
    ...
    def start_paper(self, paper_name: str, total_sections: int) -> ExtractionProgress:
        ...
        self._current_state = self._current_state.with_current_paper(paper_name, total_sections)
        
        # Update main task description
        if self.progress and self.paper_task is not None:
            self.progress.update(
                self.paper_task,
                description=f"Processing {paper_name} ({self._current_state.completed_papers + 1}/{self._current_state.total_papers})"
            )
            
            # Add section progress task if we have sections
            if total_sections > 0:
                self.section_task = self.progress.add_task(
                    f"└─ Detecting sections...",
    ...
    def complete_paper(self, elements_extracted: int) -> ExtractionProgress:
        ...
        if self.progress and self.paper_task is not None:
            self.progress.update(
                self.paper_task,
                advance=1,
                description=f"Completed {self._current_state.current_paper or 'paper'} ({elements_extracted} elements)"
            )
    ...
    def finish(self) -> None:
        ...
        if self._current_state:
            ...
            self.console.print(f"📊 Processed {self._current_state.completed_papers} papers in {self._current_state.elapsed_time:.1f}s")
            
            if self._current_state.completed_papers > 1:
                avg_time = self._current_state.elapsed_time / self._current_state.completed_papers
