...
def _find_section_end(full_text: str, start_pos: int) -> int:
    ...
    for pattern in next_header_patterns:
        matches = re.finditer(pattern, full_text[start_pos + 100:])  # Skip current header
        for match in matches:
...
def _resolve_overlapping_sections(sections: list[DetectedSection]) -> list[DetectedSection]:
    """
